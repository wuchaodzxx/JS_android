...
function refreshFileDom(fileShowElement,fileClassElementList,jsonData){
	/*
	 * 函数有三个参数
	...
	for(var i=0;i<fileClassElementList.length;i++){
		...
		//长按某项，执行该函数
			...
			mui(".mui-media-body").on('tap','.itemLink',function(){
			  focusId=this.id;
			  var FileRemark = document.getElementById(focusId).getAttribute("data-FileRemark");
			  //alert(FileRemark);
			  document.getElementById("itemMoreInfo").innerHTML=FileRemark; 
			  if(FileRemark){
			  	mui('#mui-popover-tapItem').popover('show',document.getElementById(focusId));
			  }
